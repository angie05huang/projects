...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    pick_list = [p for p in paragraphs if select(p) == True]
    if len(pick_list) <= k:
        return ''
    else:
        return pick_list[k]
...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def select(sentence):
        target = remove_punctuation(sentence).lower().split()
        for w in subject:
            for s in target:
                if w == s:
                    return True
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    dictionary = {w : diff_function(typed_word, w, limit) for w in word_list}
    if min(dictionary.values()) <= limit:
...
def time_per_word(words, timestamps_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    new_list = [[abs(x[i] - x[i+1]) for i in range(len(x) - 1)] for x in timestamps_per_player]
...
def fastest_words(match):
    ...
    for i in word_indices:
        updated_time = [time(match, j, i) for j in player_indices]
        new_list[updated_time.index(min(updated_time))].append(get_word(match, i))
    return new_list
